...
488 tracks in 47 genres, 2439.5 MB, MP3 at 44.1 and 48.0 kHz, 0:41 to 7:59.
...
_Check for new music_. The app's own music needs a build instead — it is in the bundle, and
nothing else could carry it.

_Doing it without the command: see_ **Adding a track by hand** _at the end._
...
_other_ library: everything here can be previewed immediately through one
...
ever taking away audio somebody has already downloaded. See _The catalogue_
below.
...
`tags.json` optionally assigns a collection tag (`reel` or `special`) to a
published filename, including its `.mp3` extension. The generator carries it
into both catalogues as `tag`. Collection tags do not change the genre or make
downloadable audio part of the app bundle. The Reel section includes tagged
downloads as well as the app's existing offline Reel tracks.
...
1. **an id never changes.** A project stores it, so re-titling a track is fine
   ...
   track's _audio_ under the same id is likewise a new track, not an edit;
...
4. **`schema` stays 1.** Anything new is an _optional_ key, which an older app
...
| Cinematic        | `cinematic`       | 47     | screenAndScore      |
